...
const NewRequest = React.memo(
	({ panelOpened, acceptedTokens, onRequest, connectedAccount, selectedNFT, selectNFT, auctionStatus }) => {
		const { orgTokens, nftTokens, lastSoldBlock, totalSoldNFT } = useAppLogic()
		...
		const [price, setPrice] = useState(BASE_NFT_VALUE)
				...
				if (selectedOrgToken.index === -1) {
					const index = selectedNFT.address
						? combinedOrgTokens.findIndex(
								(token) =>
									token.address === selectedNFT.address && token.tokenId === selectedNFT.tokenId
						  )
						: 0
		...
		return (
			...
			>
				<Field label="Requested amount" required>
					<CombinedInput>
						<TextInput.Number
							...
							min={
								selectedNFT.address && addressesEqual(selectedToken.value, ETHER_TOKEN_FAKE_ADDRESS)
									? price
									: 0
							}
				...
				<Info>
					...
					<p>
						Configure your request above, and sign the transaction with your wallet after clicking “Create
						request”. It will then show up in your Token request app once processed. It will need to be
						submitted by someone with permission to create proposals, you will be able to withdraw your
						funds from the request at any time before the proposal is approved.
					</p>
					{isTokenSelected && (
						<React.Fragment>
							<p
