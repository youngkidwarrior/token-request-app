...
const RequestDetail = ({ request, tokens, onBack, onSubmit, onWithdraw, nftTokens }) => {
	...
	const combineTokens = tokens.concat(nftTokens)
	const token = combineTokens.find((orgToken) => orgToken.address == requestToken)
	const depositFormated = formatTokenAmount(depositAmount, false, depositDecimals, depositDecimals)
	const requestFormated = formatTokenAmount(requestAmount, false, requestDecimals, requestDecimals)
	const requestRounded = formatTokenAmountSymbol(requestSymbol, requestAmount, requestDecimals)
								...
								{connectedAccount ? (
									status === requestStatus.PENDING && (
										<React.Fragment>
											...
											/>
											<Info mode="warning">
												This request is pending, A user with permission to initiate the approval
												process must take action. The status will not update unless it is
												approved or withdrawn by the requestor.
											</Info>
										</React.Fragment>
									)
								) : (
