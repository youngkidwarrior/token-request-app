...
async function requestRefunded(state, { requestId }) {
	const { requests, nftTokens } = state
	const { requestToken, requestTokenId, requestName, requestSymbol } = requests.find(
	...
	if (nftSelected) {
		...
		nftTokens = [
			...nftTokens,
			{ address: requestToken, tokenId: requestTokenId, name: requestName, symbol: requestSymbol, uri },
		]
	}
	return {
		...state,
