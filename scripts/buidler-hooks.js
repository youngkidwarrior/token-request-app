let tokens, agent, testToken, accounts
...
module.exports = {
  postDao: async function({ _experimentalAppInstaller, log }, bre) {
    ...
    await agent.createPermission('TRANSFER_ROLE', voting.address)
    await agent.createPermission("EXECUTE_ROLE", voting.address)
    await agent.createPermission("RUN_SCRIPT_ROLE", voting.address)
  },
  getInitParams: async function({}, bre) {
    const tokenList = [ZERO_ADDRESS, testToken.address]

    return [[tokens.address], agent.address, tokenList]
  },
